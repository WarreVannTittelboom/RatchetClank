...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		...
		m_ControllerDesc.position.x = this->GetTransform()->GetPosition().x;
		m_ControllerDesc.position.y = this->GetTransform()->GetPosition().y;
		m_ControllerDesc.position.z = this->GetTransform()->GetPosition().z;
