...
void SpriteTestScene::Initialize()
{
	...
	m_pSprite2->AddComponent(new SpriteComponent(L"Textures/TestSprite.jpg", { 0.5f,0.5f }, { 1.f,1.f,1.f,.5f }));
	AddChild(m_pSprite2);
	...
	m_pSprite2->GetTransform()->Translate(m_SceneContext.windowWidth / 2.f, m_SceneContext.windowHeight / 3.f, .9f);
	m_pSprite2->GetTransform()->Scale(1.f, 0.5f, 1.f);
}

void SpriteTestScene::Update()
{
	m_pSprite->GetTransform()->Rotate(0.f, 0.f, m_TotalRotation);
	m_pSprite2->GetTransform()->Rotate(0.f, 0.f, m_TotalRotation/2.f);
