...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	specularangle = max(dot(halfwayDir,normal),0.0);
	float specular = pow(specularangle,gShininess);
	float3 specularColor = gColorSpecular * specular * specularIntensity;
	return float3(specularColor);
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specAngle = saturate(dot(R, viewDirection));
	specular = pow(specAngle, gShininess);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	{
	float4 textureSample = gTextureNormal.Sample(gTextureSampler, texCoord);
	...
	float3 NormalValues = textureSample.rgb;
	float3 sampledNormal = float3(2.f * NormalValues.x - 1.f ,2.f * NormalValues.y - 1.f ,2.f * NormalValues.z - 1.f);
	newNormal = mul(sampledNormal, tangentSpaceAxis);
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	{
		float3 textureDiffuse = gTextureDiffuse.Sample(gTextureSampler,texCoord);
		textureDiffuse *= diffuseStrength;
		textureDiffuse *= gColorDiffuse;
		return float4(textureDiffuse ,1);
	}
	else
	{
...
{
	float3 environment = float3(1,1,1);
	if(gUseEnvironmentMapping)
	{
	float3 refractVec = refract(normal, viewDirection, gRefractionIndex); //gRefractionStrength
	float3 environmentSampleRefract = gCubeEnvironment.Sample(gTextureSampler,refractVec);
	float3 reflectVec = reflect(normal,viewDirection); //gReflectionStrength
	float3 environmentSampleReflect = gCubeEnvironment.Sample(gTextureSampler,reflectVec);
	environment = environmentSampleReflect * gReflectionStrength + environmentSampleRefract * gRefractionStrength;
	}
	return float3(environment);
}
